...
const PeopleDetails = () => {
  ...
  const {data: actor,isLoading,error,} = useFetch({queryKey: ["getPeopleDetails", id],queryFn: () => getPeopleDetails(id as string)});
  const {data: actorMovies} = useFetch({queryKey: ["getPeopleMovies", id],queryFn: () => getPeopleMovies(id as string)});

  if (isLoading)
    return (
      <div className="h-screen flex items-center justify-center bg-black text-white">
        <div className="animate-pulse text-lg">Loading actor profile...</div>
      </div>
    );
          ...
          <div className="mt-14">
            ...
            <div className="flex flex-wrap gap-2">
              {actor.also_known_as.map((name: string, i: number) => (
                <span key={i} className="px-3 py-1 bg-white/10 border border-white/10 rounded-full text-sm hover:bg-white/20 transition">
                  {name}
                </span>
              ))}
            </div>
