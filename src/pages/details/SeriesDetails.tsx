...
const SeriesDetails = () => {
  ...
  const { data: details } = useFetch({
    ...
    queryFn: () => getSeriesDetails(id as string),
  });
  const { data: seriesTrailer } = useFetch({
    queryKey: ["getSeriesTrailer", id],
    queryFn: () => getSeriesTrailer(id as string),
  });
  ...
  const imgScr = details.backdrop_path
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      <div className="relative w-full h-[70vh]">
        ...
        <div className="absolute bottom-0 p-6 md:p-10 max-w-4xl">
          ...
          <p className="text-gray-300 mt-3 max-w-2xl">{details.tagline}</p>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 grid md:grid-cols-3 gap-10">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm text-gray-300">
            <p>
              <span className="text-gray-500">First Air:</span>{" "}
          ...
          <div className="flex flex-wrap gap-2">
            {details.genres?.map((g: { id: string; name: string }) => (
              <span
                key={g.id}
      ...
      {trailers.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 py-10">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {trailers
              .slice(0, 4)
              .map((trailer: { id: string; name: string , key:string}) => (
                <div
                  key={trailer.id}
